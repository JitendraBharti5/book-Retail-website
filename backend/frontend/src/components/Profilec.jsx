...
const handleDelete = async (bookId) => {
  if (window.confirm("Are you sure you want to delete this book?")) {
    try {
      ...
      fetchUserBooks(); // Refresh the book list
    } catch (err) {
  ...
  const onSubmit = async (data) => {
    try {
      const res = await axios.put(`/book/${editBook._id}`, data);
...
<img src="w.png" alt="image"/>
 ...
 <div className="pl-20">
      <div>
        <span>Name: </span>
        <span>{authUser?.fullname || "N/A"}</span>
      </div>
      <br />
      ...
      <div className="num e">
        ...
        <span>{authUser?.contact || "N/A"}</span>
      </div>
      <br />
      <div className="loc e">
        <span>State: </span>
        <span>{authUser?.state || "N/A"}</span>
      </div>
      <br />
    </div>
<Link to="/sell">
